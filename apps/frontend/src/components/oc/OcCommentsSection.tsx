...
function formatSystemLine(content: string): string {
  ...
  text = text.replace(
    /^([A-Za-zÀ-ÿ' .]+?) (criou|aprovou|enviou|rejeitou|excluiu)/i,
    (_, name: string, verb: string) => `${shortPersonName(name)} ${verb}`
  );
  return text;
}

type OcCommentsSectionProps = {
  purchaseOrderId: string;
  currentUserId?: string | null;
};

export function OcCommentsSection({ purchaseOrderId, currentUserId }: OcCommentsSectionProps) {
  const queryClient = useQueryClient();
  const [text, setText] = useState('');
  const queryKey = ['purchase-order-comments', purchaseOrderId] as const;

  const { data: items = [], isLoading } = useQuery({
    queryKey,
    queryFn: async () => {
      const res = await api.get(`/purchase-orders/${purchaseOrderId}/comments`);
      return (res.data?.data ?? []) as OcFeedItem[];
    },
    enabled: !!purchaseOrderId,
  });

  const groupedByDay = useMemo(() => {
    const groups: Array<{ dayKey: string; label: string; items: OcFeedItem[] }> = [];
  ...
  const postMutation = useMutation({
    mutationFn: async (content: string) => {
      const res = await api.post(`/purchase-orders/${purchaseOrderId}/comments`, { content });
      return res.data.data as OcFeedItem;
    },
    onSuccess: (comment) => {
      queryClient.setQueryData<OcFeedItem[]>(queryKey, (prev) => [
        ...(prev ?? []),
    ...
    onError: (error: { response?: { data?: { message?: string } }; message?: string }) => {
      toast.error(
        error.response?.data?.message || error.message || 'Não foi possível comentar'
      );
    },
  });
  ...
  const deleteMutation = useMutation({
    mutationFn: async (commentId: string) => {
      await api.delete(`/purchase-orders/comments/${commentId}`);
      return commentId;
    },
    onSuccess: (commentId) => {
      queryClient.setQueryData<OcFeedItem[]>(queryKey, (prev) =>
