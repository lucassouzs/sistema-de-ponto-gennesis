...
export function OcCommentsSection({ purchaseOrderId, currentUserId }: OcCommentsSectionProps) {
  ...
  const { data: comments = [], isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      return (res.data?.data ?? []) as OcComment[];
    },
  ...
  const postMutation = useMutation({
    ...
    onSuccess: (comment) => {
      queryClient.setQueryData<OcComment[]>(queryKey, (prev) => [...(prev ?? []), comment]);
      setText('');
    },
  ...
  const deleteMutation = useMutation({
    ...
    onSuccess: (commentId) => {
      queryClient.setQueryData<OcComment[]>(queryKey, (prev) =>
  ...
  return (
    <div className="space-y-3">
      ...
      <textarea
        ...
        rows={2}
        className={`${FORM_FIELD_TEXTAREA_CLS} mb-0 !min-h-0 py-2 text-sm`}
        onKeyDown={(e) => {
