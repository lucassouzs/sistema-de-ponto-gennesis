/** Espelha o encoding do backend em UserLoginEvent.source. */
...
export function parseLoginEventSource(source?: string | null): {
  ...
  if (raw.startsWith('{')) {
    try {
      ...
      const channel =
        parsed.channel === 'mobile' ? 'mobile' : parsed.channel === 'web' ? 'web' : raw;
      return {
        channel,
...
export function loginEventTypeLabel(type?: string | null, source?: string | null): string {
