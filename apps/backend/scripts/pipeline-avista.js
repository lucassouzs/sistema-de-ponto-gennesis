#!/usr/bin/env node
/**
 * Pipeline completo — fluxo AVISTA (criação → FINALIZED).
 *
 * Uso (padrão 20 OCs):
 *   node scripts/pipeline-avista.js
 *
 * Parâmetros:
 *   --iterations=20 --vus=5
 *   ITERATIONS=20 VUS=5 BASE_URL=http://localhost:5000/api
 */
...
const steps = [
  ...
  {
    name: 'Cotação AVISTA → OC',
    script: 'teste-carga-cotacao.js',
    env: (cfg) => ({
      ITERATIONS: String(cfg.iterations),
      VUS: String(cfg.vus),
    }),
  ...
  {
    name: 'Pagamento AVISTA → FINALIZED',
    script: 'teste-carga-pagamento-oc.js',
    env: (cfg) => ({
      ITERATIONS: String(cfg.iterations),
      VUS: String(cfg.vus),
    }),
